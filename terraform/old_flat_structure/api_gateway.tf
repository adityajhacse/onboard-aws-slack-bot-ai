...
resource "aws_api_gateway_method" "onboard_post" {
  ...
  http_method   = "POST"
  authorization = "NONE" # Consider adding AWS_IAM or CUSTOM authorizer for production
...
resource "aws_api_gateway_integration" "status_step_functions" {
  ...
  integration_http_method = "POST"
  type                    = "AWS"
  uri                     = "arn:aws:apigateway:${local.region}:states:action/DescribeExecution"
  credentials             = aws_iam_role.api_gateway.arn
...
# Update IAM role for API Gateway to include DescribeExecution
resource "aws_iam_role_policy" "api_gateway_step_functions_describe" {
  name   = "${local.name_prefix}-api-gateway-describe-execution"
  role   = aws_iam_role.api_gateway.id
  policy = data.aws_iam_policy_document.api_gateway_step_functions_describe.json
}

data "aws_iam_policy_document" "api_gateway_step_functions_describe" {
  statement {
    effect = "Allow"
    actions = [
    ...
    resources = [
      "arn:aws:states:${local.region}:${local.account_id}:execution:${aws_sfn_state_machine.onboarding.name}:*"
    ]
  }
}
