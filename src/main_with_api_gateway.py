# Modified version of main.py that uses API Gateway + Step Functions
# This replaces the synchronous processing with async workflow execution
...
# Orchestrator uses api_client so chat-flow approvals trigger Step Functions
...
def handle_det_summary_submit(ack, body, view, client):
    """
    Modified handler that triggers API Gateway instead of processing synchronously.
    """
    ...
    # Acknowledge with processing view
    ack(
        response_action="update",
        view=build_processing_view(project_name),
    )

    # Trigger API Gateway to start Step Functions workflow
    try:
        logging.info(f"Triggering onboarding workflow for {project_name}")
        ...
        # Post message to channel with execution details
        if channel_id:
            try:
                client.chat_postMessage(
                    channel=channel_id,
                ...
                logging.warning(f"Could not post workflow start message: {exc}")
        ...
        client.views_update(
            ...
            view=build_processed_confirmation_view(
                success=False,
                project_name=project_name,
                detail=f"Failed to start onboarding workflow: {str(exc)}",
            ),
        )
